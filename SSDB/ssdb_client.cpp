...
Status SSDBClient::auth(const string &key)
{
...
Status SSDBClient::info(std::vector<std::string> *ret)
{
    m_request->appendStr("info");
    m_request->endl();
...
Status SSDBClient::setx(const string &key, const string &val, int ttl)
{
    m_request->appendStr("setx");
    m_request->appendStr(key);
    m_request->appendStr(val);
    m_request->appendInt64(ttl);
    m_request->endl();
...
Status SSDBClient::del(const string &key)
{
...
Status SSDBClient::multiDel(const std::vector<string> &keys)
{
...
Status SSDBClient::incr(const string &key, int64_t incrby, int64_t *ret)
{
    m_request->appendStr("incr");
    m_request->appendStr(key);
    m_request->appendInt64(incrby);
    m_request->endl();

    request(m_request->getResult(), m_request->getResultLen());

    return read_int64(m_reponse, ret);
}
...
Status SSDBClient::hdel(const std::string &name, const std::string &key)
{
    m_request->appendStr("hdel");
    m_request->appendStr(name);
    m_request->appendStr(key);
    m_request->endl();
...
Status SSDBClient::multiHdel(const std::string& name, const std::vector<std::string> &keys)
{
    m_request->appendStr("multi_hdel");
    m_request->appendStr(name);
    for (auto& v : keys)
    {
        m_request->appendStr(v);
...
Status SSDBClient::scan(const string &key_start, const string &key_end, uint64_t limit, std::vector<string> *ret)
{
    m_request->appendStr("scan");
    m_request->appendStr(key_start);
    m_request->appendStr(key_end);
...
Status SSDBClient::hclear(const string &name)
{
    m_request->appendStr("hclear");
    m_request->appendStr(name);
    m_request->endl();
